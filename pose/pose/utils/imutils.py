...
def draw_labelmap(img, pt, sigma, type='Gaussian', scale=3):
    # Draw a 2D gaussian 
    ...
    if scale == -1:
        ...
        scale_size = scale*sigma
    # Check that any part of the gaussian is in-bounds
    ul = [int(pt[0] - scale_size), int(pt[1] - scale_size)]
    br = [int(pt[0] + scale_size + 1), int(pt[1] + scale_size + 1)]
    if (ul[0] >= img.shape[1] or ul[1] >= img.shape[0] or
            br[0] < 0 or br[1] < 0):
        # If not, just return the image as is
        return to_torch(img)
    if sigma == 0:
        img[int(pt[0]), int(pt[1])] = 1
        return to_torch(img)
    # Generate gaussian
    size = 2 * scale_size + 1
